...
/obj/item/lcl_duel/afterattack(atom/A, mob/living/user, proximity_flag, params)
	...
	RegisterSignal(agent, COMSIG_ATOM_ATTACK_ANIMAL, PROC_REF(AbnoAttack))
...
/obj/item/lcl_duel/proc/AgentAttacking(datum/source, atom/A, proximity)
	...
	if(A != linked_abno)
		return
	addtimer(CALLBACK(src, PROC_REF(FighterHealthCheck)), 1) //I'm taking the lazy way out and make a slight delay on the heal check just in case some of the damage wasn't calculated.

/obj/item/lcl_duel/proc/AbnoAttack(datum/source, mob/living/user)
	SIGNAL_HANDLER
	if(user != linked_abno)
		return
...
/obj/item/lcl_duel/proc/EndDuel()
	...
	UnregisterSignal(agent, list(COMSIG_HUMAN_MELEE_UNARMED_ATTACK, COMSIG_ATOM_BULLET_ACT, COMSIG_PARENT_ATTACKBY, COMSIG_ATOM_ATTACK_ANIMAL))
